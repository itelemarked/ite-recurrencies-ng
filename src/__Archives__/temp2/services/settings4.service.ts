...
// implements SettingsServiceInterface 
{
  ...
  private authService = inject(Auth4Service)
  ...
  private _settings$ = new BehaviorSubject<DataRequest<Settings>>({state: 'loading'})
  ...
  private _settingsChange$ = this.authService.user$.pipe(
    switchMap((usr: DataRequest<User>): Observable<DataRequest<Settings>> => {
      if(usr.state === 'loading') return of({state: 'loading'})
